...
# -------------------------------------------------------------------------
# [适配 25.12 核心修改]
# 移除 repositories.conf 修改逻辑 (25.12 使用 apk，不读取此文件)
# 生成 APK 索引 (packages.adb)，否则 ImageBuilder 无法识别本地包
# -------------------------------------------------------------------------
echo "⚡️ Generating APK index for local packages (Required for OpenWrt 25.12)..."
if [ -d "/home/build/immortalwrt/packages" ]; then
    cd /home/build/immortalwrt/packages
    # 检查是否存在 ipk 文件
    if ls *.ipk 1> /dev/null 2>&1; then
        # 生成索引并允许未信任 (本地包无官方签名)
        apk index -o packages.adb --allow-untrusted *.ipk
        echo "✅ APK index (packages.adb) generated successfully."
    else
        echo "⚠️ Warning: No .ipk files found in packages directory, skipping index generation."
    fi
    cd - > /dev/null
else
